...
var replaceTempTypeToTrue = function (modelName, columnName) {
...
var generateFunctionForRule = function (ruleSign) {
...
//==========================MODELS======================================
    var modelPromptPromiseFunc = function (resolve, reject) {
      var modelPromptOptions = {
        type: 'input',
        name: 'model',
        message: 'Enter model name' + (minContent ? "(empty field to proceed)" : ""),
        default: ''
      };
    ...
    var afterModelPrompt = function (modelName) {
      if (modelName) {
        ...
        if (!minContent) {
          _this.log('There must be minimum one model and minimum on column to proceed');
          return modelPrompt();
        }
      ...
      var columnTypePromiseFunc = function (resolve, reject) {
        ...
        var columnTypePromptCallback = function (answers) {
          ...
          switch (columnType) {
            ...
            //  break;
            case "DATE":
              //models[params.modelName].columns[params.columnName].type = "INTEGER";
              models[params.modelName].columns[params.columnName].iosType = "Date";
              resolve(params);
              break;
            default :
      ...
      var columnAllowNullPromiseFunc = function (resolve, reject) {
        ...
        var columnAllowNullPromptCallback = function (answers) {
          ...
          if (answers.doNotAllowNull)
            models[modelName].columns[columnName].allowNull = !answers.doNotAllowNull;
          resolve(params);

        };
        _this.prompt(columnAllowNullPromptOptions, columnAllowNullPromptCallback);
      };
    ...
    var columnDoValidatePrompt = function (params) {
      var columnDoValidatePromiseFunc = function (resolve, reject) {
        var modelName = params.modelName;
        var columnName = params.columnName;
        var validationTypes = constants.constantsForTypes[models[modelName].columns[columnName].type].fieldValidateTypes;
      ...
      var columnValidationTypesPromiseFunc = function (resolve, reject) {
        ...
        var columnValidationTypesPromptCallback = function (answers) {
          ...
          switch (answers.validationType) {
            ...
            default :
              params.loop = true;
              models[modelName].columns[columnName].validate[answers.validationType] = true;
              resolve(params);
          }
...
//==========================TABLES COLUMNS FAKER TYPE================
    ...
    var tablesColumnsFakerTypeModelFieldTypePrompt = function (params) {
      ...
      var formAllowedFakerTypesVorFieldValidation = function (modelName, fieldName) {
        ...
        var fieldValidations = models[modelName].columns[fieldName].validate;
        ...
        for (var i = 0; i < fieldValidationNames.length; i++) {
          if (allowedFakerTypesVorValidations[fieldValidationNames[i]].FakerTypes.length > 0) {
            allowedFields = arrayIntersection(allowedFields, allowedFakerTypesVorValidations[fieldValidationNames[i]].FakerTypes);
          }
        }
      ...
      var tablesColumnsFakerTypeModelFieldTypePromiseFunc = function (resolve, reject) {
        ...
        var addFakeType = function (type) {
          ...
          replaceTempTypeToTrue(modelName, columnName);
        };
            ...
            for (var i = 0; i < fieldValidationNames.length; i++) {
              for (var j = 0; j < validationSubstitutionName.length; j++) {
                if (fieldValidationNames[i] == validationSubstitutionName[j].name) {
                  ...
                  _this.log(validationSubstitutionName[j].index,
                      Number.isInteger(validationSubstitutionName[j].index),
                      fieldValidationValue[validationSubstitutionName[j].index]);

                  _this.log(Number.isInteger(validationSubstitutionName[j].index)
                      && fieldValidationValue[validationSubstitutionName[j].index]);
                  ...
                  if (validationValue) {
                    ...
                    var ruleFunction = generateFunctionForRule(ruleSign);
                    console.log(substitution, ruleSign, validationValue);
          ...
          if (substitutionRegExp.test(type)) {
            ...
            var allowedFakerTypesSubstitutionCallback = function (answer) {
              ...
              if (isValidationPassed) {
                ...
                _this.log('Value does not pass validations for this field');
              }
              resolve(params);
            };
