...
exports.validateNumberRange = function (rangeStr) {
  var isRnge = /^$|^\d+$|^\d+,\d+$/.test(rangeStr)
  if (!isRnge) {
    return "range must contain only numbers";
  }
  var range = rangeStr.split(',');
  if (range.length <= 1) return true;
  return parseInt(range[0]) <= parseInt(range[1]) || "min value of range cannot bi bigger than max value";
};
