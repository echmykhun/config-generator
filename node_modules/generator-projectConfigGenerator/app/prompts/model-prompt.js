...
module.exports = function (gen, next) {
  ...
  var columnTypePrompt = function (params) {
    ...
    var launchDecimalTypePrompt = function (resolve) {
      var rangePromptOptions = {
        type: 'input',
        name: 'fieldRange',
        message: 'Enter range values, separated be comma(Ex: 10,20)',
        default: '',
        validate: util.validateNumberRange
      };
      var rangePromptCallback = function (answers) {
        var range = [];
        if (answers.fieldRange.length) {
          range = answers.fieldRange.split(',');
          range.forEach(function (element, index) {
            range[index] = parseInt(element);
          });
          generator.models[params.modelName].columns[params.columnName].length = range;
        }
        resolve({modelName: params.modelName, columnName: params.columnName})
      };
      generator.prompt(rangePromptOptions, rangePromptCallback);
    };
    ...
    var columnTypePromiseFunc = function (resolve, reject) {
      ...
      var columnTypePromptCallback = function (answers) {
        ...
        switch (columnType) {
          ...
          //  break;
          case "DECIMAL":
            launchDecimalTypePrompt(resolve);
            break;
          case "DATE":
