...
module.exports = function (gen, next) {
  ...
  var relationshipsFields = constants.relationshipsFields;
  ...
  var canCreateNestedModelPrompt = function (params) {
    ...
    return new Promise(canCreateNestedModelPromisFunc).then(generateReletionshipsFieldPromptsChain(params.rel, 0));
  };


  var generatePromptForField = function (field, next) {
    ...
    return function (params) {
      var promiseFunc = function (resolve, reject) {
        ...
        var ownerClassPropPromptCallback = function (answers) {
          params.relationship[field.name] = answers[field.name];
          resolve(params)
        };
        generator.prompt(ownerClassPropPromptOptions, ownerClassPropPromptCallback);

      }
      return new Promise(promiseFunc).then(next);
    }

  }


  var generateReletionshipsFieldPromptsChain = function (type, fieldIndex) {

    var index = fieldIndex || 0;
    var field = relationshipsFields[type][index];
    if (!field) return relationsPrompt;
    return generatePromptForField(field, generateReletionshipsFieldPromptsChain(type, index + 1));
